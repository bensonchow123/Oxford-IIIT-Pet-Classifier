...
device = torch.device("cuda:7") # cuda:1 is gpu 1, cuda:2 is gpu 2, etc

# Training:
class SEResBlock(nn.Module): # Resblock but focus on specific channels
    def __init__(self, channels, reduction=16):
        ...
        self.se = nn.Sequential( # the se block 
    ...
    def forward(self, x):
        ...
        return self.relu(x + out * scale) # normal resblock + se block scale
...
# use the cross entropy loss function with label smoothing, instead of like 1 or 0 for classification, make it less definitive
criterion = nn.CrossEntropyLoss(label_smoothing=0.1)

# AdamW optimizer for the loss function
optimizer = torch.optim.AdamW(model.parameters(), weight_decay=1e-4)

EPOCHS = 30
NUM_CLASSES = 37
MIXUP_OR_CUTMIX_START_EPOCH = 20
...
cutmix = CutMix(num_classes=NUM_CLASSES, alpha=0.1)
mixup = MixUp(num_classes=NUM_CLASSES, alpha=0.1)
...
for epoch in range(EPOCHS):
    ...
    use_augmentation = epoch >= MIXUP_OR_CUTMIX_START_EPOCH
    
    for inputs, labels in tqdm(training_loader, desc="Training", leave=False): 
        inputs, labels = inputs.to(device), labels.to(device) # move data to the vram so it faster
        
        if use_augmentation:
            inputs, labels = cutmix_or_mixup(inputs, labels) # add cutmix_or_mixup
        ...
        if use_augmentation:
            correct_train += (predicted == labels.argmax(dim=1)).sum().item() # add up the softlabels
        else:
            correct_train += (predicted == labels).sum().item() # count correct
