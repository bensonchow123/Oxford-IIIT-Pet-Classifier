...
class OxfordPetClassifierCNN(nn.Module):
    def __init__(self, num_classes=37):
        ...
        self.classifier = nn.Sequential(
            ...
            nn.Linear(768, num_classes) # map the 768 features to 37 breed scores
        )
...
# This is needed for data loaders using multiple cpu cores to work on windows
if __name__ == "__main__":
    # manual seed for more consistant results
    torch.manual_seed(0)

    # load the trainval dataset for training
    raw_data = datasets.OxfordIIITPet(root=".", split='trainval', download=True)
    ...
    model_save_path = "pet_classifier_weights.pth"

    # just in case the gpu not working in cse/270
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu") # cuda:1 is gpu 1, cuda:2 is gpu 2, etc
